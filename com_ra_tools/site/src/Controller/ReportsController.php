    ...
    public function duffWalks() {
        ...
        if ($count > 0) {
            ...
            foreach ($rows as $row) {
                ...
                $objTable->add_item($toolsHelper->buildLink($target . $row->walk_id, $row->walk_id, true));
...
//           $. $row->walk_id;
    ...
    public function noLeader() {
        ...
        foreach ($rows as $row) {
            ...
            $objTable->add_item($toolsHelper->buildLink($link . '&all=Y', $count));

            $link = $target . $row->group_code;
            $objTable->add_item($toolsHelper->buildLink($link, $row->num));
            $total2 += $row->num;
    ...
    public function noLeaderDetails() {
        ...
        foreach ($rows as $row) {
            ...
            $objTable->add_item($row->group_code);
//           $. $row->walk_id;
            $objTable->add_item($toolsHelper->buildLink($target . $row->walk_id, $row->walk_id,));
            $objTable->add_item($row->walk_date);
            $objTable->add_item($row->title);
...
//        var_dump($event->location);
        echo "<h2>Event $id from WalksManager</h2>";
...
//        echo $event->title . '<br>';
        ...
        $details = $id;
        $details .= $JsonHelper->showEventButton($id);
        ...
        $objTable->add_item($toolsHelper->buildLink($event->external_url, $event->external_url, true));
        $objTable->generate_line();

        $objTable->generate_table();
        $back = 'index.php?option=com_ra_tools&task=reports.showEvents&code=' . substr($group_code, 0, 2);
        echo $toolsHelper->backButton($back);
    }
    ...
    public function showEvents() {
        ...
        $code = Factory::getApplication()->input->getAlnum('code', 'NS03');
        $callback = Factory::getApplication()->input->getWord('callback', '');
        if (strlen($code) == 2) {
            $details = $code . ' ' . $toolsHelper->lookupArea($code);
            $back = 'index.php?option=com_ra_tools&task=area.showArea&code=' . $code;
        } else {
            $details = $code . ' ' . $toolsHelper->lookupGroup($code);
            $back = 'index.php?option=com_ra_tools&task=area.showGroup&code=' . $code;
        }
...
//
//        }
        echo '<h2>Events from WalksManager for ' . $details . '</h2>';
        $objTable->add_header("Date,Time,Group,id,Title,Organiser,GR,W3W");

        $event_list = $JsonHelper->getJson('group-event', 'groups=' . $code);
...
//                $end_time = substr($event->end_date_time, 11, 5);
            ...
            $objTable->add_item($toolsHelper->buildLink($target . $event->id, $event->id));
    ...
    public function showWalks() {
        ...
        $name = $this->toolsHelper->lookupGroup($code);
...
//$objTable->add_item($row->group_code . ',' . $row->leader_user_id);
            ...
            $objTable->add_item($row->distance_miles);
            $objTable->add_item($row->contact_display_name);
            $objTable->add_item($this->toolsHelper->buildLink($target . $row->walk_id, $row->walk_id));
...
//        $sql .= "LEFT JOIN __ra_profiles as profile on profile.user_e = walks.contact_display_name ";
    ...
    public function showLogfile() {
        ...
        echo $this->toolsHelper->buildButton("index.php?option=com_ra_tools&task=reports.showLogfile&offset=" . $previous_offset, "Previous day", False, 'grey');
        if ($next_offset >= 0) {
            echo $this->toolsHelper->buildButton("index.php?option=com_ra_tools&task=reports.showLogfile&offset=" . $next_offset, "Next day", False, 'teal');
        }
        $target = "index.php";
        echo $this->toolsHelper->backButton($target);
    }
...
//=====================================================================================
    public function showWalk() {
         ...
         * 21/04/25 CB will not work unless class Walk is present
         */
...
//$objTable->num_columns = 3;
        ...
        if ($isSuperuser) {
            ...
            $details .= $toolsHelper->imageButton("I", "http://www.ramblers.org.uk/api/lbs/walks/" . $walk_id, True);
        }
        $JsonHelper = new JsonHelper;
        echo $JsonHelper->showWalkButton($id);
        $objTable->add_column($details, "L");
        $objTable->generate_header();
...
// These next items of data are returned by Walk.php, not WalkBase.php
        ...
        /*
          ...
          $details .= $toolsHelper->imageButton("M", "index.php?option=com_ra_wf&view=walk&id=" . $objWalk->id);
          }
          }
          ...
          $details = $toolsHelper->buildLink($target, $this->count_emails);
          } else {
          ...
          $details = $toolsHelper->buildLink($target, $this->count_followers);
          ...
          $details .= $toolsHelper->buildLink($target_email . '&mode=MM', "Send email to other Followers", False, "link-button button-p0186");
          }
          }
          }
          if (($this->user->id > 0) AND ($this->count_followers > 0)) {
          if ($objWalk->leader_user_id == $this->user->id) {
          $details .= $toolsHelper->buildLink($target_email . '&mode=ML', "Send email to your Followers", False, "link-button button-p0186");
          }
          }
          if (($this->user->id > 0) AND ($objWalk->leader_user_id != $this->user->id)) {
          if (($already_following == 0) AND ($days_to_go > 0)) {
          $details .= $toolsHelper->buildLink($target_follow . $objWalk->id, "Follow", False, "link-button button-p0186");
          }
          }
          $objTable->add_item($details);
          $objTable->generate_line();
...
// These items of data are returned by WalkBase.php
        ...
        /*
          ...
          $details .= $toolsHelper->buildLink($target, "Show route", False);
          } else {
          ...
          $details .= $toolsHelper->buildLink("components/com_ra_wf/upload_gpx.php?token=" . $encoded, "Upload", False, "link-button button-p7474");
          }
          }
          }
          $objTable->add_item($details);
          ...
          $details .= $toolsHelper->buildLink($link, $caption, false, "link-button button-p0159");
...
//==========================================================================
    ...
    function checkGroups() {
        /*
          ...
          ";
         */
        $this->checkTable('ra_groups', $details, '');
    }
